...
// Bladerunner Mode:
var flicker = true;
var flickerIntensity = 0.01;
var flickerColor = "128,128,128"; // must use r,g,b format.
var flickerBar = "https://almondbread.cse.unsw.edu.au/bar.png";
var overlay = true;
var overlayImage = "https://almondbread.cse.unsw.edu.au/mask.png";
//var overlayImage = "https://openlearning.cse.unsw.edu.au/site_media/viewer/media/crt.png";
var overlayObj;
var bar;
...
function createInterface( ) {
    //createButton("in", "https://openlearning.cse.unsw.edu.au/site_media/viewer/media/in.png", inButtonPos, "zoom(1, center, center);");
    //createButton("out", "https://openlearning.cse.unsw.edu.au/site_media/viewer/media/out.png", outButtonPos, "zoom(-1, center, center);");
}
...
function tileURL( x, y, zoom ) {
    ...
    var tilename = 'tile_' + 'x' + center.x + "_y" + center.y + "_z" + zoom + "." + fileType + getString; //+ "?width=512&height=512&colourmap=6";
    
    return tileServer + tilename;
}
...
function drawVisibleTiles( ) {
    ...
    if ( overlay && overlayObj.complete ) {
      context.fillStyle = "rgba("+flickerColor+","+(Math.random()*flickerIntensity)+")";
        context.fillRect( 0, 0, canvas.width( ), canvas.height( ) );
        context.drawImage( overlayObj, 0, 0, canvas.width( ), canvas.height( ) );
...
function mouseUp( ev ) {
    ...
    selectionTimeout = setTimeout( enableSelection, 200 );
}
...
function setupTileViewer( canvasId, infoId ) {
    loadSettings();
    
    info = $(infoId);
    info.html('<table class="container">'+
    ...
    $("#tileViewer").css("position", "relative");
    $(".controlButton").css("position", "absolute");
    $(".controlButton").css("left", "0px");
    $("#zoomIn").css("top", "20px");
    $("#zoomOut").css("top", "45px");
    updateInfo( );
    ...
    //canvas.css("margin-right","7px");
    //$("#tileViewer").css("margin-left","auto");
    //$("#tileViewer").css("margin-right","auto");
    //info.css("font-family", '"Courier New", monospace');
    //$(".info").css("width", '100');
    ...
    mouse = {x: size.x/2, y: size.y/2};
    topLeft = {x: -size.x/2, y: -size.y/2};
    context = canvas[0].getContext( "2d" );

    imageCache = new LRUCache( 100, imageLoader );
...
function loadSettings( ev ) {
    if (typeof(window["tileViewerSettings"]) != "undefined") {
        for (var i in tileViewerSettings) {
            window[i] = tileViewerSettings[i];
        }
    }
    
    if (limitZoom) {
        sliderStart = {x: 10, y: 30};
        sliderEnd = {x: 20, y: sliderStart.y+sliderLength};
        sliderNotchWidth = 5;
        
        sliderNotches = maxZoom - minZoom + 1;
        sliderStep = (sliderEnd.y-sliderStart.y)/(sliderNotches - 1);
...
$(document).ready(function() {
    if ($("#tileViewer").size() == 0) {
        $(document.body).append('<center><div id="tileViewer"></span></center>');
    }
    $("#tileViewer").html('<canvas id="viewerCanvas" width="800" height="600" style="border-radius: 8px; margin: 8px;"></canvas><div id="viewerInfo"></div><button type="button" class="controlButton" id="zoomIn">+ ENHANCE</button><button type="button" class="controlButton" id="zoomOut">- ABSTRACT</button>');
    $(document.body).append('<style>h1 {color: #fff; size: 24pt;} #tileViewer { width: 1045px; height: 600px; } body { font-size: 10px; font-family:monospace; background:black; color: rgb(192, 125, 0); } .controlButton { min-width:75px; color: #bbb; font-family:monospace; font-size: 10px; border:1px solid #888; background-color: #444; border-radius: 4px; text-align:left; } .controlButton:hover { background-color: #565; } .controlButton:active { border-color: #ccc; color: #eee; } .info { color: #bbb; font-family:monospace; font-size: 10px; border:1px solid #888; background-color: #444; border-radius: 2px; } .container { border-radius: 5px; border:1px solid #666; padding:5px; background-color: #333; }</style>');
    
    setupTileViewer("#viewerCanvas", "#viewerInfo");
});